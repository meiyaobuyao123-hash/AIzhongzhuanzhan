...
|---|---|---|---|---|
| **Self-serve** | 试用者、个人开发者、indie hacker | $5-2000/月 | 多模型一个 Key，价格透明 | 充值激活后即用 / 标准路由 / 限速 60-600 RPM（按充值阶梯）/ 邀请返佣 5% |
| **Team** | 小 B / 中型企业 | ¥3000-50000/月 | 合规、稳定、可签合同 | 对公收款 / 增值税专票 / 私有渠道 / SLA 99.95% / 月结 / 专属客户经理 |

**关键策略**：**0 免费额度**。所有用户必须先充值才能使用 token。这与同行（OpenRouter $5 注册赠送 / 国内同行 $0.5-1 美元免费体验）有显著差异——我们的逻辑是把"免费成本"投入到充值返赠和邀请返佣上（更符合"持续付费用户"画像），不养"白嫖即走"用户。

### 2.1 用户旅程关键节点

```
匿名 → 注册 → 邮箱验证 → 充值（最低 $5）→ 创建 API Key → 改 base URL 跑通第一条请求
                                                                        │
                                                                        ▼
                                                                 控制台看用量 → 复充
                                                                        │
...
- 注册 → 完成首次充值：> 25%（核心瓶颈：用户首充转化率）
- 首充 → 月活（连续 2 个月内复充）：> 60%
- Self-serve → Team：> 5%（按账号数）

**为什么不做"注册送 $5"**：
1. 免费额度被薅羊毛账号吃掉（脚本批量注册），真实开发者得不到
2. 行业标杆 OpenRouter 送 $5 的同时也得防滥用（强制信用卡 / GitHub 验证）
3. 等价金额改投到"首充 1.1× 返赠"或"邀请返佣"上，**只奖励真实付费意愿**
4. 我们的预算更适合花在产品力（路由质量 / 渠道透明 / 客户端适配）上，而不是引流费
