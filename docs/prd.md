...
> **充值 10000 元，到账 9995 元；手续费 5 元，仅 0.05%。**
...
| 玩家 | 充值手续费 | 倍数差 |
|---|---|---|
| OpenRouter | 5.5%（最低 $0.8）| **110×** |
| laozhang / yunwu | 隐藏在倍率里（实际 5-30%）| 100-600× |
| ohmygpt | 不公开 | - |
| **Prism** | **0.05%（万分之五）** | 基线 |

**为什么这么低**：0.05% 仅够覆盖**对公转账银行手续费**（几块钱/笔，正好 ≈ 万分之五）。支付宝 / 微信支付的 0.6-1% 通道费由我们自己吸收，**前期亏运营成本是预期内**。

### 1.2 收入来源（修正后的预期分布 Y2）

| 来源 | 占比预期 | 说明 |
|---|---|---|
| 充值手续费 | **5%** | 万分之五 × Y2 GMV 5 亿 ≈ 25 万元/年（覆盖支付通道+对公转账成本，几乎不盈利） |
...
**为什么 0 免费额度 + 万 5 手续费的组合**：
1. **免费额度的钱不如直接降手续费**：OpenRouter 模式是"送 $5 + 收 5.5% 手续费"，本质是用一笔小补贴换 5.5% 长期溢价。我们反过来——**不送但永久收万 5**，对真实付费用户更友好（充 1 万只损失 5 元，不是 550 元）
...
|---|---|---|---|---|
| 加价模式 | 0% + 充值 5.5% | 倍率分组 0.7-1× | ~0.86-1× | **0% + 充值万 5（0.05%）** |
...
| 充值手续费万分之五 | 2026-05-07 | 充 10000 到 9995，0.05% | 业内最低；前期亏运营成本，靠企业服务长期变现 |
