...
### 1.1 充值手续费：**1.5%**（2026-05-09 起）

> **2026-05-09 调整**：早期版本曾承诺"万分之五"，但 0.05% 仅够覆盖对公银行电汇成本，
> 在支付宝/微信商户号 0.6% / Stripe 3.5% / 链上 gas 等真实通道下我们每笔都亏。
> 调整为 1.5% 全通道统一，覆盖各通道实际成本 + 极小运营 margin。

我们对外的承诺：

> **充值 $10000，到账 $9850；手续费 $150（1.5%）。**
> Inference 全部按上游真实成本结算，无加价。

这个数字仍然比同行便宜：

| 玩家 | 充值手续费 | 我们对比 |
|---|---|---|
| OpenRouter | 5.5%（信用卡）/ 5%（USDC）| **3.7× 便宜** |
| laozhang / yunwu | 隐藏在倍率里（实际 5-30%）| 3-20× 便宜 |
| ohmygpt | 不公开 | - |
| **Prism** | **1.5%（全通道统一）** | 基线 |

**为什么 1.5%**：对外公开的真实成本测算 ——
- USDC 链上 gas: ~0.01% (TRC20)
- 支付宝/微信商户费: 0.6%
- 对公银行: ~0.1%
- Stripe: ~3.5%
- 全通道统一收 1.5%，刚好覆盖加权平均成本 + 留极小 margin 让模式可持续。

### 1.2 收入来源（修正后的预期分布 Y2）

| 来源 | 占比预期 | 说明 |
|---|---|---|
| 充值手续费 | **15%** | 1.5% × Y2 GMV 5 亿 ≈ 750 万元/年（覆盖通道 + 商户 + 链上 gas，~0.5% margin） |
| 企业 SLA / 私有渠道 | **55%** | Team 套餐 ¥5K-50K 元/月，是真正的现金流主力 |
| 可观测 / 数据增值 | **30%** | 高级面板、按 Project 拆分、长期日志保留、批处理、Embedding 索引 |
| 工具 / 应用 SaaS（Y3+）| **10%** | Cursor / Claude Code 类工具自营、知识库托管等延伸 |

**关键判断**：MVP 到 Y2 期间，**充值手续费基本 = 通道成本，不贡献利润**。利润全部来自企业服务和增值服务。这是**长期主义**模型，前 12-18 个月几乎不赚钱（甚至亏运营），靠口碑和透明度抢占心智，等规模到了再变现。

### 1.3 不做的收入

- ❌ inference 加价（即使 1.05× 也不做）
- ❌ 高于 1.5% 的充值手续费（1.5% 是品牌承诺，写进合同）
- ❌ 模型暗中降级
- ❌ 私扣余额
- ❌ 不透明渠道
...
OpenRouter 用 0% 加价 + 5.5% 手续费做到年化 inference 流水 1 亿美元 + 累计融资 6010 万美元（参见 [`docs/competitor-analysis.md`](competitor-analysis.md) §2.1）。我们的差异化是把"5.5%"压到"万分之五"——OpenRouter 已经验证了"低加价 + 高规模"路径走得通；我们把它推到极致，用更彻底的透明换更快的口碑扩散。
...
**为什么 0 免费额度 + 1.5% 手续费的组合**：
1. **免费额度的钱不如直接降手续费**：OpenRouter 模式是"送 $5 + 收 5.5% 手续费"，本质是用一笔小补贴换 5.5% 长期溢价。我们反过来——**不送但永久收 1.5%**，对真实付费用户更友好（充 1 万只付 150，不是 550）
2. **防薅羊毛**：免费额度会被脚本批量注册账号吃掉，真实开发者拿不到
3. **品牌锚点**：「万分之五」是个超级具体的数字，比「便宜」「实惠」「行业最低」这种空话更可信
4. **预算用在产品力**：等价的引流费改投在路由质量 / 渠道透明 / 客户端适配 / 故障率，长期 ROI 更高
...
|---|---|---|---|---|
| 加价模式 | 0% + 充值 5.5% | 倍率分组 0.7-1× | ~0.86-1× | **0% + 充值 1.5%** |
...
| 0 免费额度 | 2026-05-07 | 注册不送 token，必须充值才能用 | 防薅羊毛 + 预算改投产品力 |
| 充值手续费 1.5% | 2026-05-09 | 充 10000 到 9850 (从 0.05% 调整) | 覆盖支付通道 + 商户 + 链上 gas 实际成本 |
| 用户档位收敛为 2 档 | 2026-05-07 | Self-serve / Team | 简化运营，self-serve 内部按累计充值阶梯做运行时差异化 |
