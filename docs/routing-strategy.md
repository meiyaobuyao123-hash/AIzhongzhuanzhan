...
| **成本透明度** | 单 token 卖给客户的价 = 上游单价（误差 ≤ 0.5% 由对账验证）| 月成本 / 月 token，与上游账单对齐 |
| **配额利用率** | 单 channel 75% < 利用率 < 95% | 太低浪费、太高易触发限速 |

> 不做加价。客户看到的单价 = 上游 API 的真实定价。0.5% 是对账容差（流式断开等边界情况），不是利润空间。详见 [`docs/token-accounting.md`](token-accounting.md)。

---

## 0.4 当前阶段（v0.1 实况）vs 目标态（v0.3+）

**重要：本文档大部分章节描述的是 v0.3+ 完整路由能力**。当前 v0.1 阶段的实际情况要简单得多——

```
v0.1 实况（2026-05）:
  Anthropic: 1 个个人开发者 API 账号 → 1 条 channel
  OpenAI:    1 个个人开发者 API 账号 → 1 条 channel
  Gemini:    1 个 AI Studio API Key  → 1 条 channel

  总共 3 条 channel，每个模型对应唯一 1 条。
  没有多渠道权重选择、没有 Bedrock、没有 Vertex、
  没有 cooldown/熔断（单 channel 出问题就直接 502）、
  没有 cache 粘性、没有用户档位分流（只有一个池）。

  路由 = "查 model 字段 → 找到唯一 channel → 转发"
```

**v0.1 路由层只做 3 件事**：
1. 用 model 字段反查它属于哪个 provider，命中那条 channel
2. 流式转发上游响应给客户端
3. 解析上游 `usage` 字段，按 model 单价精确计费

剩下 §1-§9 的 4 层路由 / 价格²反比 / cache 粘性 / 容量预警，全部是 **v0.2-v1.0 的目标设计**。等多账号、Bedrock 折扣、企业私有渠道这些需求出现时，再分批落地。

之后本文档其他章节默认描述的是 v0.3 完整能力，请知悉。
